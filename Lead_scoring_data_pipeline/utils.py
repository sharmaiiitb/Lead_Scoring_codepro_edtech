...
def build_dbs():
    ...
    if os.path.isfile(DB_PATH+DB_FILE_NAME):
        print("DB is Already Exsist")
        print(os.getcwd())
        return "DB Exsists"
    else:
        print("Creating the Database")
        """ creating a the database connection to a SQLite database """
        conn = None
        try:
...
def load_data_into_db():
    ...
    print("Reading the csv file: ", datafile)
    df_lead_scoring = pd.read_csv(datafile)
    
    print("Replacing null values present in the 'toal_leads_dropped' and 'referred_lead' with 0")
    ...
    print("Saving processed dataframe in the db in the table named 'loaded_data")
...
def map_city_tier():
    ...
    print("Mapping cities according  to their tier")
    ...
    print("Dropping the city_mapped column")
    loaded_data = loaded_data.drop(['city_mapped'], axis=1)
    
    print("Saving the processed dataframe in the db in the table named 'city_tier_mapped'")
    loaded_data.to_sql(name='city_tier_mapped', con=cnx,
...
def map_categorical_vars_column(input_df, column_name, column_values):
    # get row level which are not present in the column_values
    new_df = input_df[~input_df[column_name].isin(column_values)] 
    
    # replacing the value of these levels to others for convinent
    new_df[column_name] = "others" 
    
    # get row for level which are present in column_values
    old_df = input_df[input_df[column_name].isin(column_values)] 
    
    # concatenating the new_df and old_df to getting the final df
...
def map_categorical_vars():
    ...
    print("Reading the data from the mapped_df")
    mapped_df = pd.read_sql('select * from city_tier_mapped', cnx)
    
    print("Mapping all the unsignificant variables present in first_platform_c")
    mapped_df = map_categorical_vars_column(mapped_df, 'first_platform_c', list_platform)
    
    print("Mapping all the unsignificant variables present in first_utm_medium_c")
    mapped_df = map_categorical_vars_column(mapped_df, 'first_utm_medium_c', list_medium)
    
    print("Mapping all the unsignificant variables present in first_utm_source_c")
    mapped_df = map_categorical_vars_column(mapped_df, 'first_utm_source_c', list_source)
    
    print("Saving processed df in the db in a table named 'categorical_variables_mapped'")
    mapped_df.to_sql(name='categorical_variables_mapped', con=cnx, if_exists='replace',index=False)
...
def interactions_mapping():
    ...
    print("Reading the data from the categorical_variables_mapped")
    categorical_variables_mapped = pd.read_sql('select * from categorical_variables_mapped', cnx)
    
    # read the interaction mapping file
    print("Reading the interaction_mapping from categorical_variables_mapped")
    ...
    print("Unpivoting interaction columns and put values in the rows")
    ...
    print("Handling  nulls in the interaction value column")
    df_unpivot['interaction_value'] = df_unpivot['interaction_value'].fillna(0)
    
    print("Mapping the interaction type column with the mapping file to get interaction mapping")
    df = pd.merge(df_unpivot, df_event_mapping, on='interaction_type', how='left')
    
    print("Dropping  interaction type column as its having not much needed")
    df = df.drop(['interaction_type'], axis=1)
    
    print("Pivoting interaction mapping column values to the individual columns in the dataset")
    df_pivot = df.pivot_table(values='interaction_value', index=INDEX_COLUMNS,                                                     columns='interaction_mapping', aggfunc='sum')
    df_pivot = df_pivot.reset_index()
    
    print("Saving processed df in the db in a table named 'interactions_mapped'")
