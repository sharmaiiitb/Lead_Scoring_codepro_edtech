...
def encode_features():
    ...
    **NOTE : You can modify the encode_featues function used in heart disease's inference
    ...
    print("Printing the data: ", df_model_input.head())
    ...
    print("Encoded dataframe to columns: ", df_encoded.columns)
    
    df_encoded.to_sql(name='features', con=cnx,
                       if_exists='replace', index=False)
    # saveing the features and target in the separate tables
...
def get_trained_model():
    ...
    print("MLflow tracking url and create/set experiment")
    create_mlflow_experiment()
    
    print ("Connecting to the database")
    cnx = sqlite3.connect(DB_PATH+DB_FILE_NAME)
    print ("Reading the data the from features table")
    X = pd.read_sql('select * from features', cnx)
    print ("Reading the data from the target table")
    y = pd.read_sql('select * from target', cnx)
   
    print ("Splitting the data in train and test set")
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.2, random_state = 100)
    
    mlflow_run_name = ML_FLOW_RUN_NAME+date.today().strftime("%d_%m_%Y_%H_%M_%S")
    print ("Starting the mlflow run: ", mlflow_run_name)
    with mlflow.start_run(run_name=mlflow_run_name, nested=True) as run:
        print("Instantiated the LGBMClassifier")
        clf = lgb.LGBMClassifier()
        print("Setting the model configurations: ", model_config)
        clf.set_params(**model_config)
        print("Starting the LGBMClassifier model training")
        clf.fit(X_train, y_train)
        
        print("Logging the model to mlflow with name LightGBM")
        mlflow.sklearn.log_model(sk_model=clf, artifact_path="models", registered_model_name='LightGBM')
        print("Logging the model params in the mlflow")
        ...
        print("To Compute metrics on test data")
        ...
        print("List of Logging metrics in MLflow")
...
def create_mlflow_experiment():
    ...
    # Setting  environment with the created experiment
