...
def encode_features():
    ...
    # Implementing these steps to preventing the dimension mismatch during inference
    for feature in encoded_df.columns:
        if feature in df_model_input.columns:
            encoded_df[feature] = df_model_input[feature]
        if feature in placeholder_df.columns:
            encoded_df[feature] = placeholder_df[feature]
    # filling all null values
    encoded_df.fillna(0, inplace=True)
    
    print("Encoded dataframe columns are: ", encoded_df.columns)
...
def get_models_prediction():
    ...
    model_load = mlflow.pyfunc.load_model(model_uri=f"models:/{MODEL_NAME}/{STAGE}")
    
    predictions = model_load.predict(pd.DataFrame(X))
    pred_df = X.copy()
